...
export class MotorIA {
     ...
     */
    static processarTurno(inimigos, alvo, onAcaoRealizada) {
    ...
    static moverParaAlvo(inimigo, alvo, todosInimigos) {
        ...
        if (dist > 1) {
            ...
            if (novoX < 0 || novoX >= LARGURA_CAMPO)
                novoX = inimigo.x;
            if (novoY < 0 || novoY >= ALTURA_CAMPO)
                novoY = inimigo.y;
    ...
    static moverFugindo(inimigo, alvo, _todosInimigos) {
        ...
        if (novoX < 0)
            novoX = 0;
        if (novoX >= LARGURA_CAMPO)
            novoX = LARGURA_CAMPO - 1;
        if (novoY < 0)
            novoY = 0;
        if (novoY >= ALTURA_CAMPO)
            novoY = ALTURA_CAMPO - 1;
        inimigo.x = novoX;
        inimigo.y = novoY;
    }
    static atacarCorpoACorpo(inimigo, alvo, onAcaoRealizada) {
        const dist = distanciaEntre({ x: inimigo.x, y: inimigo.y }, { x: alvo.x, y: alvo.y });
        if (dist <= 1) {
            const dano = Math.floor(inimigo.atk);
            alvo.receberDano(dano);
            inimigo.tocarSomAtaque();
            onAcaoRealizada(`${inimigo.nome} atacou! Causou ${dano} de dano.`);
        }
    }
    static atacarDistancia(inimigo, alvo, alcance, onAcaoRealizada) {
        const dist = distanciaEntre({ x: inimigo.x, y: inimigo.y }, { x: alvo.x, y: alvo.y });
        if (dist <= alcance) {
